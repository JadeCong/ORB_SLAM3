    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    // bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile); // load VocFile from txt(by JadeCong)
    bool bVocLoad = mpVocabulary->loadFromBinaryFile(strVocFile); // load VocFile from binary(by JadeCong)
    if(!bVocLoad)
    {
